module.exports = (function() {
    ...
    authTokens.start = function(playerId, trackingCode) {
        return games.read('', 'admin', {
                trackingCode: trackingCode
            }, true)
            .then(function(game) {
                if (game) {
                    var gameplays = new Collection(require('./db'), 'gameplays_' + game._id);
                    return gameplays.find({
                        ...
                        var authToken = game._id + gameplay._id + token() + gameplay.sessions;
