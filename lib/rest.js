...
module.exports = function (router, options) {
    ...
    router.post(collectorRoot + 'start/:trackingCode', function (req, res) {
        ...
        if (!req.session || !req.session.userId) {
            promise = players.findByAuthorization(req.headers.authorization);
        } else {
            promise = players.findByUserId(req.session.userId);
        }

        processResponse(promise.then(function (player) {
            return collector.start(req.params.trackingCode, player);
        }), res);
    });
