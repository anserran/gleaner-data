module.exports = function (db, router, options) {
    ...
    if (Array.isArray(options.extraConsumers)) {
        var traces = require('./traces');
        for (var i = 0; i < options.extraConsumers.length; i++) {
            traces.addConsumer(options.extraConsumers[i]);
        }
    }
    ...
    router.get(apiRoot + 'sessions/', find(sessions));
