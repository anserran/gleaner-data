module.exports = (function () {
    ...
    var collection = new Collection(require('./db'), 'versions');
    collection.sort = {
        _id: -1
    };
    ...
    var permissions = {
        'create': {
            'admin': true,
            'developer': function (userId, version) {
                var games = require('./games');
                return games.readResource(userId, 'developer', version.gameId);
            }
        },
        'read': {
            'admin': true,
            'developer': function (userId) {
                var games = require('./games');
                return games.read(userId, 'developer').then(function (results) {
                    var ids = [];
        ...
        'update': {
            'admin': true,
            'developer': function (userId, versionId) {
                return collection.findById(versionId).then(function (version) {
                    var games = require('./games');
                    return games.readResource(userId, 'developer', version.gameId);
                });
            }
    ...
    return new Resource('versions', collection, permissions);
})();